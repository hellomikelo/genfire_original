...
function obj = refineOrientation_parallel_REFINEClass(obj)
...
ncx       = round((dim1+1)/2);
ncy       = round((dim2+1)/2);
nc_padded1 = round((size(modelK,1)+1)/2);
nc_padded2 = round((size(modelK,1)+1)/2);
...
parfor pj_num = 1:num_proj
    ...
    pj    = projections(:,:,pj_num);
    phi   = phiangles(pj_num);
    theta = thetaangles(pj_num);
    psi   = psiangles(pj_num);
    calc_count    = 1;
    
    % because of MATLAB's parfor rules have to create separate variables
    ...
    for cur_phi = (phi+phi_search_range)
            for cur_theta = (theta+theta_search_range)
                    for cur_psi = (psi+psi_search_range)
                        ...
                        calc_pj = forwardProjection_func(modelK,cur_phi,cur_theta,cur_psi); % get projection
                        %calc_pj = My_stripzero(calc_pj,[dim dim]);
                        calc_pj = calc_pj(cropInd1,cropInd2).*FPmask(:,:,pj_num);
                        [metric, suggested_center_x, suggested_center_y] = compare_func(pj,calc_pj); % compare projections
